...
#                                                                              #
# Original code Copyright (C) 2005-2006 Laurent Pinchart && Michel Xhaard     #
#                    2007 Lucas van Staden                                     #
#                    2007 Tom Stöveken                                         #
#                                                                              #
# Optimizations Copyright (C) 2024 350d                                       #
#                                                                              #
...
int input_init(input_parameter *param, int id)
{
    ...
    /* Set context pointer for direct copy optimization */
    ...
    /* Set frame metadata in global structure for optimization */
    pglobal->in[id].width = pctx->videoIn->width;
    pglobal->in[id].height = pctx->videoIn->height;
    pglobal->in[id].format = pctx->videoIn->formatIn;
    pglobal->in[id].fps = pctx->videoIn->fps;
    pglobal->in[id].quality = settings->quality;
    pglobal->in[id].current_size = 0;  /* Will be set on each frame */
    pglobal->in[id].prev_size = 0;     /* Will be set on each frame */
    
    /* Initialize frame synchronization fields */
    pglobal->in[id].frame_timestamp_ms = 0;  /* Will be set on first frame */
    pglobal->in[id].frame_sequence = 0;      /* Start from 0 */

    /* Initialize performance optimizations */
    if(init_optimized_select(pctx) < 0) {
        IPRINT("Failed to initialize optimized select\n");
    }
    
    /* Initialize optimized timestamp handling */
    init_optimized_timestamp(pctx, fps > 0 ? fps : 5);
    
    /* Initialize TurboJPEG handle caching */
    if(init_turbojpeg_handle(pctx) < 0) {
        IPRINT("Failed to initialize TurboJPEG handle\n");
    }
    
    /* Initialize YUV buffers for compression */
    if(init_yuv_buffers(pctx, width, height) < 0) {
        IPRINT("Failed to initialize YUV buffers\n");
    }
...
int input_run(int id)
{
    ...
    if(pthread_create(&(pctx->threadID), NULL, cam_thread, in) != 0) {
        IPRINT("Failed to create camera thread for input %d\n", id);
        return -1;
    }
    ...
    while(!pglobal->stop) {
        ...
        if (FD_ISSET(pcontext->videoIn->fd, &pcontext->rd_fds)) {
            ...
            if(wantTimestamp)
            {
                ...
                get_optimized_timestamp(pcontext, &pcontext->videoIn->tmptimestamp);
            }
            ...
            /* Check if frame was already processed by direct copy optimization */
            if (pcontext->videoIn->direct_copy_used) {
                /* Frame already processed - reset flag and skip processing */
                pcontext->videoIn->direct_copy_used = 0;
                goto other_select_handlers;
            }

            /* Optimized frame processing with minimal mutex locking */
            ...
            (pcontext->videoIn->formatIn == V4L2_PIX_FMT_RGB565) ) {
                ...
                /* Use optimized YUV compression with cached handle */
                compressed_size = compress_yuv_to_jpeg_optimized(pcontext, pcontext->videoIn, pglobal->in[pcontext->id].buf, pcontext->videoIn->framesizeIn, quality);
            } else {
            #endif
                DBG("copying frame from input: %d\n", (int)pcontext->id);
                /* Use traditional memcpy for MJPEG */
                compressed_size = memcpy_picture(pglobal->in[pcontext->id].buf, pcontext->videoIn->tmpbuffer, pcontext->videoIn->tmpbytesused);
            #ifndef NO_LIBJPEG
            }
            #endif
...
other_select_handlers:
        ...
        if (dv_timings) {
            if (FD_ISSET(pcontext->videoIn->fd, &pcontext->wr_fds)) {
                IPRINT("Writing?!\n");
            }

            if (FD_ISSET(pcontext->videoIn->fd, &pcontext->ex_fds)) {
                IPRINT("FD exception\n");
...
void cam_cleanup(void *arg)
{
    ...
    cleanup_optimized_select(pctx);
    cleanup_turbojpeg_handle(pctx);
    cleanup_yuv_buffers(pctx);
