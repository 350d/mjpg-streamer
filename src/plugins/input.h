...
struct _input {
    ...
    /* Frame metadata for optimization */
    int width;                      /* Frame width */
    int height;                     /* Frame height */
    int format;                     /* Frame format (V4L2_PIX_FMT_*) */
    int fps;                        /* Frames per second */
    int quality;                     /* JPEG quality (if applicable) */
    int current_size;                /* Current frame size in bytes */
    int prev_size;                   /* Previous frame size for comparison */
    volatile unsigned int frame_sequence;     /* Frame sequence number for multi-consumer detection */
    long long frame_timestamp_ms;    /* Timestamp of the frame in milliseconds */

    /* Relay system fields removed - no longer used */
