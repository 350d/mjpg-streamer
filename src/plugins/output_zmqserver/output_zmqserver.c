...
static int fd, ringbuffer_size = -1, ringbuffer_exceed = 0;
...
void worker_cleanup(void *arg)
{
    ...
    // No local frame buffer to free - using global buffer directly
    close(fd);
...
void *worker_thread(void *arg)
{
    ...
    while(ok >= 0 && !pglobal->stop) {
        ...
        /* OPTIMIZATION: Use global buffer directly without copying */
        /* This saves memory and CPU cycles by avoiding memcpy and multiple buffers */
        unsigned char *current_frame = pglobal->in[input_number].buf;

        /* copy v4l2_buffer timeval to user space */
        timestamp = pglobal->in[input_number].timestamp;

        /* CRITICAL: Keep mutex locked until we finish using the frame data */
        /* This prevents race conditions with other plugins modifying the buffer */
        ...
        if (mjpgFileName == NULL) { // single files with ringbuffer mode
            ...
            if ((buf == NULL) || (bufferSize < (frame_size * zmqBufferSize + 20)))
            {
                bufferSize = (frame_size * zmqBufferSize + 20);
            ...
            pbPackage.frame[zmqBufferPos]->blob.data = current_frame;
            ...
            if (zmqBufferPos == zmqBufferSize)
            {
                ...
                DBG("packing data: %i %i", frame_size, len);
            ...
            /* Now we can safely release the mutex after using the frame data */
            ...
            //if(write(fileno(stdout), current_frame, frame_size) < 0) {
            if(fwrite(current_frame, sizeof(unsigned char), frame_size, stdout) < 0) {
            ...
            /* Release mutex after processing frame */
            pthread_mutex_unlock(&pglobal->in[input_number].db);
        }
    }
