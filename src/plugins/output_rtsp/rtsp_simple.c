/*******************************************************************************
# Simple RTSP Server for MJPG-streamer                                       #
#                                                                              #
# This plugin implements a basic RTSP server for streaming MJPEG              #
...
#define OUTPUT_PLUGIN_NAME "Simple RTSP Server"
...
int send_rtp_packet(rtsp_client_t *client, unsigned char *data, int size);
void help(void);
...
int send_rtp_packet(rtsp_client_t *client, unsigned char *data, int size)
{
    ...
    rtp_addr.sin_port = htons(5004); // Default RTP port
...
int handle_rtsp_request(rtsp_client_t *client, char *request)
{
    ...
    if (strcmp(method, "PLAY") == 0) {
        snprintf(response, sizeof(response),
                ...
                "CSeq: %d\r\n"
                "Session: %d\r\n"
                "RTP-Info: url=rtsp://127.0.0.1:%d/stream;seq=0;rtptime=0\r\n"
                "\r\n", RTSP_VERSION, cseq, session_id, rtsp_port);
        return send(client->socket, response, strlen(response), 0);
    }
    ...
    while (server_running && !pglobal->stop) {
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            if (!clients[i].active) {
                ...
                pthread_create(&client_thread, NULL, rtsp_client_thread, &clients[i]);
