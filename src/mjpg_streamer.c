...
#                                                                              #
#      Original code Copyright (C) 2007 Tom Stöveken                           #
...
static void signal_handler(int sig)
{
    ...
    /* cleanup input plugins and relay system */
    for(i = 0; i < global.incnt; i++) {
        /* cleanup condition variables */
        pthread_cond_destroy(&global.in[i].db_update);
        pthread_mutex_destroy(&global.in[i].db);
        
        /* close input plugin handles */
...
int main(int argc, char *argv[])
{
    ...
    global.in = calloc(global.incnt, sizeof(struct _input));
    if (global.in == NULL) {
        LOG("ERROR: could not allocate input array\n");
        closelog();
        exit(EXIT_FAILURE);
    }
    ...
    for(i = 0; i < global.incnt; i++) {
        ...
        char *space_pos = strchr(input[i], ' ');
        
        
        tmp = (space_pos != NULL) ? (size_t)(space_pos - input[i]) : strlen(input[i]);
        
        
        /* Initialize all fields of input structure */
        global.in[i].stop      = 0;
        global.in[i].context   = NULL;
        global.in[i].buf       = NULL;
        global.in[i].size      = 0;
        global.in[i].name      = NULL;
        global.in[i].handle    = NULL;
        global.in[i].in_parameters = NULL;
        global.in[i].parametercount = 0;
        global.in[i].width     = 0;
        global.in[i].height    = 0;
        global.in[i].format    = 0;
        global.in[i].fps       = 0;
        global.in[i].quality   = 0;
        global.in[i].current_size = 0;
        global.in[i].prev_size = 0;
        global.in[i].frame_timestamp_ms = 0;
        global.in[i].frame_sequence = 0;
        ...
        global.in[i].param.parameters = NULL;
        global.in[i].param.argc = 0;
        global.in[i].param.global = &global;
        
        /* Initialize param.argv array to prevent memory corruption */
        for (j = 0; j < MAX_PLUGIN_ARGUMENTS; j++) {
