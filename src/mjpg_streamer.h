...
#else
// Define basic types and structures for non-Linux systems
...
// V4L2 structure stubs for non-Linux systems
...
/* Wait for a new frame using sequence number and timed wait */
int wait_for_new_frame(void *in_ptr, unsigned int *last_sequence);
...
struct _globals {
    ...
    /* input plugin */
    input *in; // Changed from array to pointer to avoid incomplete type error
    int incnt;
...
/* Relay system function declarations removed - no longer used */
